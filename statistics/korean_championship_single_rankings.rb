...
class KoreanChampionshipSingleRankings < GroupedStatistic
  def initialize
    @title = "Korean Championship single rankings"
    @note = "Top 10 Korean competitors by event at Korean Championships."
  ...
  def transform(query_results)
    Events::OFFICIAL.map do |event_id, event_name|
      results = query_results
        ...
        .each { |result| result["single"] = SolveTime.new(event_id, :single, result["single"]) }
        .sort_by! { |result| result["single"] }
        .first(10)
        .map! do |result|
          [result["single"].clock_format, result["person_link"], result["country_name"], result["competition_link"]]
        end
      [event_name, results]
    end
  end
